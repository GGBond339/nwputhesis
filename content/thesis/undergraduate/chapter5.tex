...
命中半径$50\ \mathrm{m}$（零高度）和$1000\ \mathrm{m}$（最大动压）直接采用第4章基于500组LHS样本$95\%$分位数确定的命中判据。角速度阈值$1.0\ \mathrm{rad/s}$由估算得到。最高点高度阈值$1500\ \mathrm{m}$参照猎户座PA-1试验经验~\cite{mcgowan2011overview}：该试验远地点约$1800\ \mathrm{m}$，成功完成降落伞展开与着陆，因此$1500\ \mathrm{m}$可作为最低安全高度的参考下限。峰值动压阈值$50\ \mathrm{kPa}$也由估算得到。
...
本章基于第四章的打靶统计结果，建立了涵盖命中精度、姿态稳定性和结构安全的三级风险评估体系。评估表明，两种工况的命中风险和姿态风险均为低风险，超限概率均低于5\%，峰值动压、最高点高度和最大角速度等非脱靶指标的超限概率均为0\%，控制裕度和结构安全裕度充足。最大动压工况的峰值动压已接近结构承载边界，尾部风险值得关注。
